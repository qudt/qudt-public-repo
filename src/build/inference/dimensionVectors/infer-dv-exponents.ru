...
#
# Handles both integer exponents (e.g. T-1) and decimal exponents encoded with
# 'dot' (e.g. L-0dot5 → −0.5) as used in CGS dimension vectors.
#
# IRI local-name formats:
#   qkdv:  A{a}E{e}L{l}I{i}M{m}H{h}T{t}D{d}
#   edv:   A{a}E{e}L{l}I{i}M{m}H{h}T{t}D{d}R{r}N{n}
#
# Instances that already carry qudt:dimensionExponentForAmountOfSubstance are
...
# Applies to both qudt:QuantityKindDimensionVector (qkdv:) and
# qudt:ExtendedDimensionVector (edv:) instances.
...
WHERE {
    GRAPH ?g {
        ...
        UNION
        { ?dv a qudt:ExtendedDimensionVector . }
...
# Applies only to qudt:ExtendedDimensionVector (edv:) instances.
# Does not fire until qudt:ExtendedDimensionVector is declared in the schema
# and edv: instances are added to the vocabulary.
