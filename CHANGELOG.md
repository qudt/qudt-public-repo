...
- Added `qudt:organizedUnder`, an organizational/unit-applicability grouping relation (a sub-property of `skos:broader`) that does **not** assert commensurability. Applicable-unit inference traverses `skos:broader` (and hence `qudt:organizedUnder`); commensurability is derived separately, from `qudt:specializationOf` and `qudt:exactMatch`.
