...
- Corrected the quantity kind of `constant:BohrMagneton`, `constant:NuclearMagneton`, `constant:BohrMagnetonInEVPerT` and `constant:NuclearMagnetonInEVPerT` from `quantitykind:MagneticDipoleMoment` (weber metre) to `quantitykind:MagneticMoment` (joule per tesla, the dimension of their values), and clarified the Bohr and nuclear magneton descriptions.
- Corrected the quantity kind of the nine particle magnetic moment constants whose values are in J/T (`constant:ElectronMagneticMoment`, `constant:ProtonMagneticMoment`, `constant:NeutronMagneticMoment`, `constant:MuonMagneticMoment`, `constant:DeuteronMagneticMoment`, `constant:TritonMagneticMoment`, `constant:ShieldedHelionMagneticMoment`, `constant:ShieldedProtonMagneticMoment` and `constant:AtomicUnitOfMagneticDipoleMoment`) from `quantitykind:MagneticDipoleMoment` to `quantitykind:MagneticMoment`.
- Updated the values and standard uncertainties of the ten Bohr and nuclear magneton constants (in J/T, eV/T, Hz/T or MHz/T, m⁻¹/T and K/T) from CODATA 2006 to the 2022 CODATA recommended values.
...
- Rederived the OWL schemas with the new blank node ordering.

### Fixed

- Updated references to IEC 80000-6 for electro-magnetic quantity kinds
...
- Correct typo in `quantitykind:ElectricFlux  qudt:expression "$electric-flux$"`
...
- Corrected the applicable units of `quantitykind:Log10Ratio`. It carried `unit:DECADE`, but the decade is the unit of IEC CDD `UAD084` (the base-10 *frequency interval*); `UAD085` (logarithmic ratio to base 10) lists the **bel** and the **decibel**. `DECADE` is removed and `unit:B`/`unit:DeciB` are linked instead — they previously pointed only at the four `Sound*Level` quantity kinds and not at the generic base-10 logarithmic ratio they are the coherent units of.
- Stopped encoding the logarithm base in quantity-kind names, resolving part of [#1410]. The principle: source vocabularies determine which quantities exist and what they mean, but QUDT's commensurability model — not any one source's grouping — determines whether two source entries are the same quantity kind or two different ones. Entries related by a factor or an offset (as base variants of a logarithm always are, since `log_b(x) = ln(x)/ln(b)`) collapse into one quantity kind, with every source code preserved on it; entries that are not so related stay separate regardless of how any one source names or groups them. Applied here: IEC CDD [`UAD066` - information content](https://cdd.iec.ch/cdd/iec62720/iec62720.nsf/ListsOfUnitsAllVersions/0112-2---62720%23UAD066?opendocument) already lists the shannon, the hartley and the natural unit of information as three units of a *single* quantity; ECLASS models the same three as separate quantities, but since they differ only by base they still collapse. Accordingly `BinaryLogarithmicInformationContent`, `CommonLogarithmicInformationContent` and `NaturalLogarithmicInformationContent` are removed (they were added after 3.4.0 and never released) in favour of `quantitykind:InformationContent`, which already carried all four applicable units and now also carries their three ECLASS codes (BAJ463/BAJ468/BAJ469), with an `rdfs:comment` stating which code corresponds to which unit (shannon/hartley/nat). The same treatment is applied to the ECLASS-only median-information-flow trio — `Binary`/`Common`/`NaturalLogarithmicMedianInformationFlow` are deprecated in favour of `quantitykind:InformationFlowRate` (these *had* shipped in 3.4.0, so they are deprecated rather than removed), carrying over BAJ464/BAJ470/BAJ471 with the same per-unit guidance; "median" is dropped as a statistical property of a particular measurement rather than of the quantity kind.
- Revisited the naming for the quantity kinds that keep the base (logarithmic ratio, frequency interval), after an audit of every release since 2.1.45 (Nov 2024) and independent feedback from a colleague both favoured the older, shorter names. Those releases show `Log10Ratio`, `LogERatio`, `Log10FrequencyInterval` and the unqualified (base-2) `LogarithmicFrequencyInterval` stable throughout, coexisting with verbose duplicates of the same quantities (`LogarithmRatioToBase10`/`LogarithmRatioToBaseE`/`LogarithmicFrequencyIntervalToBase10`) that were also live in 3.4.0 — the actual inconsistency [#1410] raised. Rather than deprecate the stable short names in favour of new `Common`/`Natural`/`Binary`-prefixed ones, the short names are kept live (gaining the `eclassCode`/`iec61360Code`/`siExactMatch` and applicable units that had accumulated on their unreleased `Common`/`Natural`/`Binary` counterparts, which are removed) and the verbose duplicates are deprecated in their favour instead. IEC gives these base choices separate entries with disjoint unit sets (`UAD085` bel/decibel, `UAD086` neper, `UAD084` decade) and `Log10Ratio` additionally carries `siExactMatch si-quantity:RLGB`, so — unlike information content — the base-10/base-2/base-e split itself is retained; only the *name* changes. `TimeRelatedLogarithmicRatio` (released in 3.4.0) is deprecated in favour of the new `quantitykind:TimeRelatedLogERatio`, since its unit `NP-PER-SEC` is base-e and the convention now requires that to be explicit. `CommonLogarithmicRatioPerLength` was also never released — it was minted independently in [#1501] to consolidate the IEC/ECLASS duplicate pair `LinearLogarithmicRatio`/`LineicLogarithmicRatio`, whose own descriptions left the base unstated. Rather than keep that name, the survivor is the new `quantitykind:Log10RatioPerLength`, matching the explicit-base convention established above (its description already asserted base 10); `LinearLogarithmicRatio` and `LineicLogarithmicRatio` are both deprecated in its favour, each keeping its own `iec61360Code`/`eclassCode`. `LinearLogarithmicRatio`'s `rdfs:label`, which read "Lineic Logarithmic Ratio" — mismatching its own URI — is corrected to "Linear Logarithmic Ratio" in the process. All fifteen `dcterms:isReplacedBy` links in this family are repointed so every redirect resolves to a live quantity kind. Also minted `quantitykind:Log2FrequencyInterval` as an `exactMatch` synonym of `LogarithmicFrequencyInterval` — the unqualified name is base-2 by definition (unit `OCT`, IEC `UAD083`) but does not say so, so this offers a precise, self-documenting URI for anyone who wants it, mirroring how `unit:LB`/`unit:LB_M` coexist as an ambiguous/unambiguous pair. It carries the same `iec61360Code`, `wikidataMatch` and applicable units directly (not left to exactMatch propagation), so the external mappings and units are visible without following the link.
...
- Applied the Common/Natural/Binary logarithm naming convention to the logarithmic ratio and frequency-interval quantity kinds, deprecating the base-explicit names (`qudt:deprecated true`, `qudt:deprecatedInVersion "3.5.0"`, `dcterms:isReplacedBy`):
  ...
  - Identified `LineicLogarithmicRatio` (ECLASS) and `LinearLogarithmicRatio` (IEC 61360) as duplicates of the same base-10 per-length ratio (B/m, dB/m, dB/km — all base-10 units by definition of the bel), whose descriptions left the base unstated, and repointed `unit:B-PER-M`, `unit:DeciB-PER-M` and `unit:DeciB-PER-KiloM` accordingly. (The survivor originally minted here for this pair, `CommonLogarithmicRatioPerLength`, is itself later superseded by `quantitykind:Log10RatioPerLength` — see the logarithm-naming entry above.) No natural-log sibling is minted until requested (the natural-log per-length concept is effectively `LinearAttenuationCoefficient`, and no Np/m unit exists).
...
- Closed out the unit-less U- and V-named quantity kinds (7 of 9 in scope; `WarmReceptorThreshold` is owned by #1500's sensory-threshold family and excluded, as is `TouchThresholds`'s sibling `VisionThresholds`, left as an abstract umbrella per the same reasoning already applied to `TouchThresholds`; `quantitykind:Unknown` is the deliberate null-placeholder used across the vocabulary for units of undetermined quantity kind and is not itself a quantity to give a unit). No U-named quantity kinds needed attention.
  - Duplicates linked with `qudt:exactMatch`, so units propagate through the exactMatch closure: `VolumeDensityOfCharge` (ECLASS BAJ368) and `VolumetricElectricCharge` (IEC UAD242) → `ElectricChargeVolumeDensity` (C/m³); `VolumetricEntityDensity` (ECLASS BAJ377, "value inversely proportional to volume") → `InverseVolume` (/m³); `VolumetricOutputPower` (IEC UAD243) and `VolumicOutput` (ECLASS BAJ366) → `PowerDensity` (W/m³, already `exactMatch PowerPerVolume`).
  - `VoltagePhasor` — a complex-valued representation whose magnitude is the RMS voltage — is `organizedUnder quantitykind:Voltage` rather than `exactMatch`, since a phasor is not literally the same real-valued quantity despite sharing its unit; given the conventional engineering subset directly (`V`, `KiloV`, `MilliV`, `MicroV`) rather than all 15 of `Voltage`'s units, since `organizedUnder` inherits nothing.
  - `Wavenumber` is `organizedUnder quantitykind:InverseLength` (1/m), which several other named, mutually inconvertible quantities already share (`PhaseCoefficient`, `PropagationCoefficient`, `LinearIonization`), and is linked to `unit:PER-M` directly. Its own description notes spectroscopy conventionally uses cm⁻¹ instead; no such unit exists in the vocabulary and none is minted speculatively.
  - `VolumicAmountOfSubstance` (ECLASS BAJ402, "particles… divided by volume") is left unit-less: whether "amount of substance" here means moles (dimension `A1`, matching `AmountOfSubstancePerVolume`/`AmountOfSubstanceConcentration`) or a generic particle count (dimension `A0`, matching `NumberDensity`) is genuinely ambiguous from the ECLASS text, and the only mol/m³ node with usable direct units (`Concentration`) is itself described as spanning four different concentration types. Raised as a question rather than guessed at. `VolumicDataQuantity`'s dimension (1/m³) is already correct; no unit exists for it.
- Fixed over-claimed applicable units in the `Temperature` and `Time` families. A quantity kind with no direct `qudt:unitForQuantityKind` links inherits its `qudt:specializationOf` parent's entire unit set, which misfires when the parent's own unit list spans several domains. Twenty-three specialisations were inheriting their parent's full list — including domain-alien units such as the agricultural `DEG_C_GROWING_CEREAL` under every temperature specialisation — and now carry the units they are actually quoted in: `CelsiusTemperature`/`ColdReceptorThreshold` (°C), `FahrenheitTemperature` (°F), `DewPointTemperature`/`DryBulbTemperature`/`WetBulbTemperature` (°C, °F), `CurieTemperature`/`DebyeTemperature`/`FermiTemperature`/`NeelTemperature`/`SuperconductionTransitionTemperature`/`TemperatureDifference` (K), `CarrierLifetime` (s, µs, ns, ps), `HalfLife`/`MeanLifetime` (s, min, h, d, a), `Adaptation` (min, s), `ReactorTimeConstant`/`ReverberationTime` (s), and `ActionTime`/`BurnTime`/`IgnitionIntervalTime`/`WebTime` (s, ms).
- Repointed the viscosity-over-pressure units `POISE-PER-BAR`, `CentiPOISE-PER-BAR`, `PA-SEC-PER-BAR`, `MilliPA-SEC-PER-BAR` and `POISE-PER-PA` from `quantitykind:Time` to `quantitykind:RelaxationTime`. Dimensionally these are seconds, but η/G is the Maxwell relaxation time — a time constant for exponential decay towards equilibrium — not a general duration; nobody quotes an elapsed time in poise per bar. `RelaxationTime` already existed with a matching definition and no direct units of its own, and now also carries `SEC`/`MilliSEC`.
- Moved the `qudt:specializationOf`/`skos:broader`-across-`qudt:exactMatch` closure inference (savepoint `07.5`) to before applicable-unit inference (savepoint `02`) in the build pipeline. Applicable-unit inference traverses `qudt:specializationOf` only, and the early `unitForQuantityKind` fan-out across `exactMatch` cliques only propagates *direct* unit links — so a quantity kind whose only route to a unit-bearing ancestor ran through an `exactMatch` synonym's `specializationOf` parent ended up with no applicable units, because the closure edge that would have connected it was inferred a thousand pipeline lines after applicable units had already been computed. No currently-authored quantity kind hit this (each has an independent working path), but any future `exactMatch` onto a kind without direct units would have failed silently. The step's inputs (`exactMatch`, authored `specializationOf`, quantity-kind dimension vectors) are all available at the earlier position; dimension-vector *inference* at savepoint `07` concerns units only, so nothing the rule's dimension guard reads is lost by the move. As a side effect, extension quantity kinds that specialise core kinds now also see the closure edges during their applicable-unit run.
- Closed out the unit-less R- and S-named quantity kinds (23 of 25; the two exceptions are noted below).
  ...
  - Specialisations: `RotaryShock`→`AngularMomentum` (an angular impulse *is* a change in angular momentum, N·m·s); `SecondRadiationConstant`→`LengthTemperature` (m·K); `SpecificThrust`→`Speed`; `Susceptance`→`Admittance` (mirroring `ModulusOfAdmittance`); `SurgeImpedanceOfTheMedium`→`MassPerAreaTime`; `SymbolTransmissionRate`→`DigitRate` (a symbol rate is a narrower case of a digit rate, in baud).
  - `SpecificImpulseByWeight` is retyped from `qudt:organizedUnder` to `qudt:specializationOf quantitykind:SpecificImpulse`, inheriting seconds. `SpecificImpulse` already *is* the weight-basis convention — its own definition is $I_{sp} = F/(\dot{m} g_0)$ and its dimension is T — so the two are in fact commensurable. `organizedUnder` was not incorrect, merely weaker than the evidence supports: it leaves commensurability unasserted, whereas `specializationOf` records it. A description of the weight-vs-mass basis distinction is added, with `rdfs:seeAlso quantitykind:SpecificThrust`, which carries the mass basis (m/s, differing by $g_0$).
  - `ReciprocalPlaneAngle` is linked to `unit:PER-RAD`, which already existed but was orphaned as `categorizedByQuantityKind quantitykind:Unknown`. `SpectralLuminousEfficiency` V(λ), a ratio of radiant fluxes normalised to 1, is `organizedUnder DimensionlessRatio` with `unit:UNITLESS` and `unit:FRACTION`.
  - Corrected the swapped dimensions of the ECLASS optical-rotation pair. `SpecificOpticalRotationalAbility` (BAJ425) is defined against *Massenkonzentration* (mass concentration), so it is rad·m²/kg — corrected from `A-1E0L2I0M0H0T0D0` and made `exactMatch quantitykind:SpecificOpticalRotatoryPower`. Its sibling `MolarOpticalRotationalAbility` (BAJ426), defined against *Stoffmengenkonzentration*, carries the mirror-image error and is corrected separately in [#1501]. The names were right; the dimension vectors were on the wrong entities.
  - Corrected `SpectralIrradiance` and `SpectralEmittance` from `A0E0L-4I0M1H0T-3D0` to `A0E0L-1I0M1H0T-3D0` = W/m³, as their own descriptions ("power per unit area … per unit wavelength") and their sibling `SpectralRadiance` require, and linked the orphaned `unit:W-PER-M2-NanoM`. Likewise `SpectralConcentrationOfRadiantEnergyDensity` (ECLASS) and `SpectralRadiantEnergyDensityInTermsOfWavelength` (IEC) carried the dimension of energy; both are corrected to J/m⁴ and made `exactMatch quantitykind:SpectralRadiantEnergyDensity`, of which they are the two source-vocabulary twins.
  - Clustered the angular-frequency density-of-states entries, resolving #1504: a repo-external reviewer confirmed from the IEC CDD entries' own SI coherent units (UAD030 → s/(rad·m³) *inverse energy*; UAD178 → s/(m³·rad); UAD180 → (Hz·rad·m³)⁻¹) that IEC carries three genuinely distinct quantities under this dimension family, not one. `SpectralConcentrationOfVibrationalModes` (ECLASS BAJ431) and `quantitykind:SpectralDensityOfVibrationalModes` (IEC UAD178, now given a description) are `exactMatch quantitykind:VibrationalDensityOfStates`, all three being defined as vibrational modes per interval of *angular frequency* per volume. `StateDensityAsExpressionOfAngularFrequency` (ECLASS BAJ454 / IEC UAD180) is kept as its own root rather than folded into the same clique — same dimension vector, but IEC gives it a separate code and its own coherent unit — and `unit:SEC-PER-RAD-M3` (IEC UAB352, previously misattached to `VibrationalDensityOfStates`) is repointed to it, closing out its missing applicable unit. `VibrationalDensityOfStates` loses the `iec61360Code` `UAD030`, which belongs to `EnergyDensityOfStates` instead (IEC's coherent unit for UAD030 is inverse-energy-times-inverse-volume, not the angular-frequency dimension `VibrationalDensityOfStates` actually has) — `StateDensity` (ECLASS BAJ427), already `exactMatch quantitykind:EnergyDensityOfStates` and corrected to `A0E0L-5I0M-1H0T2D0` from `NotApplicable`, confirms the same pairing from the ECLASS side.
  - Left unit-less deliberately: `RadiantEnergyExposure`, whose name says radiant exposure (J/m²) while its dimension is that of ionizing-radiation `Exposure` (C/kg), with no description to adjudicate (raised in #1503); and `SquareEnergy` (J²), for which no unit exists.
- Gave three quantity kinds their conventional units directly, stopping them inheriting their parent's entire set. Because a quantity kind with no direct units inherits all of its `specializationOf` parent's, `SpecificThrust` had been claiming `BFT` (Beaufort) and `GigaHZ-M` from `Speed`, `SurgeImpedanceOfTheMedium` had been claiming `TONNE-PER-HA-YR` and other agricultural deposition rates from `MassPerAreaTime`, and `ResistanceBasedInductance` had been claiming all forty of `Time`'s units including `PlanckTime` and `MO_Synodic`. They now carry `M-PER-SEC`; `RAYL`/`RAYL_MKS`/`PA-SEC-PER-M`/`N-SEC-PER-M3`; and `SEC`/`MilliSEC`/`MicroSEC` respectively.
- Repointed the six henry-per-ohm units (`H-PER-OHM`, `MilliH-PER-OHM`, `MicroH-PER-OHM` and their `KiloOHM` variants) from `quantitykind:Time` to `quantitykind:ResistanceBasedInductance`. H/Ω *is* L/R, the defining expression of an inductance-based time constant, and is not a way anyone expresses time in general; `Time` retains its remaining units.
...
[#1410]: https://github.com/qudt/qudt-public-repo/issues/1410
[#1495]: https://github.com/qudt/qudt-public-repo/issues/1495
[#1497]: https://github.com/qudt/qudt-public-repo/issues/1497
