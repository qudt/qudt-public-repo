...
- Stopped encoding the logarithm base in quantity-kind names where the source vocabularies do not, resolving part of #1410. IEC CDD [`UAD066` - information content](https://cdd.iec.ch/cdd/iec62720/iec62720.nsf/ListsOfUnitsAllVersions/0112-2---62720%23UAD066?opendocument) lists the shannon, the hartley and the natural unit of information as three units of a *single* quantity, so the base belongs to the unit and not to the quantity kind; only ECLASS splits it. Accordingly `BinaryLogarithmicInformationContent`, `CommonLogarithmicInformationContent` and `NaturalLogarithmicInformationContent` are removed (they were added after 3.4.0 and never released) in favour of `quantitykind:InformationContent`, which already carried all four applicable units and now also carries their three ECLASS codes (BAJ463/BAJ468/BAJ469) with an `rdfs:comment` recording the many-to-one mapping. The same treatment is applied to the ECLASS-only median-information-flow trio — `Binary`/`Common`/`NaturalLogarithmicMedianInformationFlow` are deprecated in favour of `quantitykind:InformationFlowRate` (these *had* shipped in 3.4.0, so they are deprecated rather than removed), carrying over BAJ464/BAJ470/BAJ471; "median" is dropped as a statistical property of a particular measurement rather than of the quantity kind.
