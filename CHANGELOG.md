...
- Closed out the unit-less R- and S-named quantity kinds (23 of 25; the two exceptions are noted below).
  ...
  - Clustered the angular-frequency density-of-states entries: `SpectralConcentrationOfVibrationalModes` (ECLASS BAJ431) and `StateDensityAsExpressionOfAngularFrequency` (ECLASS BAJ454 / IEC UAD180) are now `exactMatch quantitykind:VibrationalDensityOfStates`, all three being defined verbatim as vibrational modes per interval of *angular frequency* per volume. `quantitykind:SpectralDensityOfVibrationalModes` (IEC UAD178) is deliberately left out: it carries no definition, and per-angular-frequency and per-frequency densities of state share a dimension vector because the radian is dimensionless, so merging on unit-symbol evidence alone is unsafe — see #1504. `StateDensity` (ECLASS BAJ427) is separate on the same grounds but unambiguously so: it is defined per the *energy* interval, giving it a different dimension, and its `NotApplicable` is corrected to `A0E0L-5I0M-1H0T2D0` with `exactMatch quantitykind:EnergyDensityOfStates`.
