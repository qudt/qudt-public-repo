...
- Gave the remaining unit-less B-named quantity kinds commensurability links: `BandwidthDistanceProduct` `specializationOf` `BandwidthLengthProduct` (with `unit:MegaHZ-KiloM` retyped from `Unknown` to `BandwidthLengthProduct`), and `BinaryLogarithmicMedianInformationFlow` `specializationOf` `InformationFlowRate` (inheriting the entropy-rate units).
